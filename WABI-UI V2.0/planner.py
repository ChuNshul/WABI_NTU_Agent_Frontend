...
def _call_llm_for_plan(prompt: str, model: Optional[str] = None):
    try:
        ...
        msg = f"[planner] Cannot import GatewayClient: {exc}"
        logger.error(msg)
        _set_last_llm_error(msg)
        return None
    ...
    try:
        ...
        msg = f"[planner] Gateway chat.complete failed: {exc}"
        logger.error(msg)
        _set_last_llm_error(msg)
        return None

    if not raw_text:
        msg = "[planner] Gateway returned empty response"
        logger.error(msg)
        _set_last_llm_error(msg)
        return None
...
def _parse_json(raw_text: str) -> Optional[Dict[str, Any]]:
    ...
    if not match:
        msg = "[planner] No JSON object in LLM output"
        logger.error(msg)
        _set_last_llm_error(msg)
        return None
    ...
    try:
        ...
        try:
            ...
            msg = f"[planner] JSON parse failed: {exc}"
            logger.error("%s | raw: %.300s", msg, json_str)
            _set_last_llm_error(msg)
            return None
