...
import csv, os, datetime, threading, zlib, base64
...
def ui_node(state: Any) -> Any:
    ...
    try:
        if html:
            ...
            try:
                compressed_img = base64.b64encode(
                    zlib.compress(img_url.encode("utf-8"))
                ).decode("ascii")
            except Exception:
                compressed_img = ""
        else:
    ...
    row = [
        ...
        int(max((t1 - t0), 0) * 1000),
        int(max((t2 - t1), 0) * 1000),
        int(max((t3 - t2), 0) * 1000),
        int(max((t4 - t3), 0) * 1000),
        int(max((t4 - t0), 0) * 1000),
        intent or "",
        user_input_str,
        upstream_str,
        compressed_img,
        _get_render_mode(state),
        "" if (updates.get("ui_image_url") or compressed_img) else (error_reason or "no_image")
    ]
    # 追加写入并立即刷盘
    with _csv_lock:
...
def ui_node_direct_html(state: Any) -> Any:
    ...
    try:
        ...
        updates["ui_image_url"] = img_url
        try:
            compressed_img = base64.b64encode(zlib.compress(img_url.encode("utf-8"))).decode("ascii")
        except Exception:
            compressed_img = ""
    except Exception as exc:
    ...
    _csv_header = [
        ...
        "prompt_ms","llm_ms","html_ms","render_ms","total_ms",
        "intent","user_input","upstream","ui_image_compressed","render_mode","error"
    ]
    with _csv_lock:
        _ensure_csv_header(_csv_path, _csv_header)
    row = [
