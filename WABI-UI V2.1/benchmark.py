...
from langgraph_app.agents.ui_render.ui_node import ui_node
...
def run_mode(items: List[Dict[str, Any]], mode: str, log: Callable[[str], None], limit: int = None) -> None:
    os.environ["WABI_UI_RENDER_MODE"] = mode
    
    # 随机抽样逻辑
    if limit and limit < len(items):
        sampled_items = random.sample(items, limit)
        log(f"[{mode}] Randomly sampled {limit} items from {len(items)} total items")
    else:
        sampled_items = items
        log(f"[{mode}] Using all {len(items)} items")
...
def main():
    ...
    limit = 10
    tee(f"Benchmark start | dataset={os.path.basename(dataset_path)} | total_items={len(items)} | limit={limit}")
    run_mode(items, "planner", log=tee, limit=limit)
    run_mode(items, "direct", log=tee, limit=limit)
