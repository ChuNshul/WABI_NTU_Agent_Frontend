...
def _render_section(section: Dict[str, Any]) -> str:
    ...
    if sec_type == "image_display":
        ...
        if sec_type == "bar_chart":
            try:
                max_val = max([float(v) for v in values]) if values else 0.0
            except Exception:
                max_val = 0.0
            ...
            for label, value in zip(labels, values):
                try:
                    ...
                    v = 0.0
                percent = int(round(max(0.0, min(1.0, v / max_val)) * 100))
                html += f'<div class="key-value-item"><span class="key">{label}</span><span class="value">{value}</span></div>'
            ...
            for label, value in zip(labels, values):
                try:
                    ...
                    v = 0.0
                percent = int(round(max(0.0, min(1.0, v / total)) * 100))
        ...
        for item in get("items", []) or []:
            label = item.get("label", "")
            value = item.get("value", "")
            html += f'<div style="background:#f9f9f9; padding:10px; text-align:center; border-radius:8px;"><div style="font-size:12px; color:#666;">{label}</div><div style="font-size:16px; font-weight:bold;">{value}</div></div>'
        html += '</div>'
...
def render_ui_plan_to_image(ui_plan: Dict[str, Any], output_dir: str, base_url: str = "") -> str:
    ...
    if debug:
        print(f"[Renderer] Rendering started")
    ...
    with sync_playwright() as p:
        try:
            ...
            raise e
    if debug:
        print(f"[Renderer] Render succeeded")
    return filename
