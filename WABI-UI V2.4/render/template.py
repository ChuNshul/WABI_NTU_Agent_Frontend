...
def _template_guardrails(*, ui_lang: str = "zh") -> str:
    ...
    resources_items = "".join(
        f'<li class="item"><div class="num">{_e(num)}</div><div class="desc">{_e(desc)}</div></li>'
...
<div class="card">
    <div class="hdr">
        <div class="logo">Wabi Assistant</div>
    ...
    <div class="resources">
        <div class="rt"><div class="rk">{t['res_title']}</div><div class="tag">{t['res_tag']}</div></div>
        <ul class="list">
            {resources_items}
        </ul>
    </div>
...
def _template_tutorial(*, ui_lang: str = "zh") -> str:
    ...
    def _items(lines) -> str:
        return "".join(
            f'<li class="item"><div class="num">{idx}</div><div class="desc">{_e(s)}</div></li>'
...
<div class="card">
    ...
    <div class="resources">
        <div class="rt"><div class="rk">📷 {_e(t["prep_title"])}</div></div>
        <ul class="list">{prep_items}</ul>
    </div>
    <div class="resources">
        <div class="rt"><div class="rk">⬆️ {_e("Upload Steps" if ui_lang == "en" else "上传步骤")}</div></div>
        <div class="grid">
            <div class="app">
                <div class="apphdr"><div class="rk wx">WeChat</div><div class="badge wx">WECHAT</div></div>
                <ul class="list">{wechat_items}</ul>
            </div>
            <div class="app">
                <div class="apphdr"><div class="rk wa">WhatsApp</div><div class="badge wa">WHATSAPP</div></div>
                <ul class="list">{wa_items}</ul>
            </div>
        </div>
    </div>
    <div class="resources">
        <div class="rt"><div class="rk">📝 {_e(t["add_title"])}</div></div>
        <ul class="list">{add_items}</ul>
    </div>
    <div class="resources">
        <div class="rt"><div class="rk">💬 {_e(t["examples_title"])}</div></div>
...
def render_template(*, intent: str, user_input: str) -> str:
    ...
    if it == "guardrails":
        return _template_guardrails(ui_lang=ui_lang)
    if it == "tutorial":
        return _template_tutorial(ui_lang=ui_lang)
    return _template_tutorial(ui_lang=ui_lang)
