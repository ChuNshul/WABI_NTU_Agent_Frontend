...
def get_correction_data(state: GraphState) -> Dict[str, Any]:
    ...
    根据平台不同采用不同的反馈收集方式：
    - Web: 立即记录反馈，显示反馈表单
    - WeChat/WhatsApp: 标记等待反馈状态，让用户在对话中输入
    ...
    if chat_history:
        ...
        # 如果上一条是等待反馈的纠错消息，当前输入即为反馈
        if last_ai_msg and last_ai_msg.get('mode') == 'correction_feedback' and last_ai_msg.get('awaiting_feedback'):
            is_follow_up_feedback = True
            print(f"[DataProvider] 检测到反馈跟进，用户输入: {user_input[:50]}")
    
    # 如果是跟进反馈，记录并回复感谢
    if is_follow_up_feedback:
        # 记录用户的详细反馈
        log_correction_feedback(
            patient_id=patient_id,
            correction_input="follow_up_feedback",
            previous_intent=previous_intent,
            previous_response=previous_response,
            feedback_type="chat_feedback",
        ...
        if language == "English":
            response = (
                "Thank you for your feedback! I have recorded your suggestions.\n\n"
        ...
        return {
            ...
            "data_source": "correction_feedback_recorded",
            "feedback_recorded": True,
            "feedback_content": user_input,
        }
    
    # 首次触发纠错：记录基础反馈并提示用户输入详细反馈
    log_correction_feedback(
        patient_id=patient_id,
    ...
    if language == "English":
        response = (
            ...
            "Please tell me what went wrong with my previous answer. "
            "For example: the food was identified incorrectly, the recommendation was not accurate, etc.\n\n"
            "Your feedback will help me improve!"
        )
    else:
        response = (
            "非常抱歉之前的回答有误。\n\n"
            "请告诉我刚才的回答哪里不对，例如：食物识别错了、推荐不准确等。\n\n"
            "您的反馈将帮助我改进服务！"
        )
    ...
    return {
        ...
        "feedback_recorded": True,
        "awaiting_feedback": True,  # 标记等待用户反馈
    }
